...
const Header_pedido = ({ 
  ...
  const array_pdf = [numero_mesa, pegar_local, rua, numero, dinheiro, cartao, pix,name_on];
  ...
  // current_date.setHours(current_date.getHours()); // Ajuste o valor conforme necessário
  ...
  useEffect(() => {
    ...
    if(pedido_itens){  
      ...
      setItens(pedido_itens.map((item: any,index:number) => {
        ...
        if(item.categoria_2 === 'lanches' || item.categoria_2 === 'hotdogs' || item.categoria_2 === 'combos'){
          return {
            numero_item: index + 1 ,
        ...
        else if( item.categoria_2 === 'porcoes' || item.categoria_2 === 'outros'){
          return {
            numero_item: index + 1 ,
  ...
  const handleSave = () => {
    ...
    setLoadingNota(true);
    axios.post(`https://api.focusnfe.com.br/v2/nfce?ref=${referencia[0]?.ref + 1}&token=ntsMiSP0TCcsECYXcvnFvBdfDatDZDzT`, nota_fiscal, {
      headers: {
  ...
  useEffect(() => {
    ...
    // console.log(current_date.toISOString().slice(0, -1)) 
  }, [referencia])
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {celular ? <Text style={styles.text}>Tel : {celular}</Text> : null}
      <View style={styles.container_info}>
        ...
        {observacao ?
          <>
            ...
            <Text style={styles.text_pagamento}>{observacao}</Text>
          </>
          : null}
      </View>

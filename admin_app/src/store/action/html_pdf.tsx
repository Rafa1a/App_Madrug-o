...
export const generateHTMLForItems = (items: any, info: any) => {
  ...
  const total_value_entrega = () => {
    if(!info[0] && !info[1]){
      return parseFloat(totalValue) + 5
    }else return parseFloat(totalValue)
  }
  const taxa_entrega = () => {
    if(!info[0] && !info[1]){
      return (`<h2 style="text-align: center; font-size: 40px; font-weight: bold; border-bottom: 2px solid #000;">Taxa de entrega: R$ 5,00</h2>`)
    }else return ( ``)
  }
  // Adiciona o total ao final do HTML
  const totalHTML = `<h2 style="text-align: center; font-size: 75px; font-weight: bold; border-bottom: 2px solid #000;">Total: R$ ${total_value_entrega()}</h2>`;
      ...
      if (item !== undefined && item !== false) {
        if (item === true && index===1) {
          ...
          return `
            ...
            <div key=${index} style="text-align: left; font-size: 50px; border-bottom: 2px dashed #000;">Pix</div>
          `;
        } else {
          if (index === 0) {
            return `<div key=${index} style="text-align: left; font-size: 50px; border-bottom: 2px dashed #000">Numero da Mesa: ${item}</div>`;
            ...
            return `
            <div key=${index} style="text-align: left; font-size: 60px;">Nome :</div>
            <div key=${index} style="text-align: left; font-size: 50px; border-bottom: 2px dashed #000;">${item}</div>`; 
          }
        }
      }

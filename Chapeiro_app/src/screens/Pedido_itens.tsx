...
const Pedido_itens = (props: any) => {
  const { route, onAtualizarPedido } = props;
  const { numero_mesa, image_on, name_on, id, ids, rua, numero, pegar_local, dinheiro, pix, cartao,observacao } = props.pedidos_itens;

  const atualizar_pedido = () => {
    onAtualizarPedido(id)
...
const mapStateProps = ({ pedidos,}: { pedidos:any}) => {
  return {
      pedidos_itens: pedidos.pedidos_itens,
  };
};
